...
void* dlops::resolve_do(std::string name)
{
    if (nullptr == handle)
        return nullptr;
...
bool dlops::try_resolve_do(std::string name, void** outptr)
{
    if (nullptr == handle)
        return false;
...
std::unordered_map<std::string, dlops*> dlops::active_libraries;
dlops& dlops::get(std::string str)
{
    auto res = active_libraries.find(str);
    if (res == active_libraries.end())
    {
        auto ptr = active_libraries[str] = new dlops(str);
        return *ptr;
    }
    else
    {
...
void dlops::close(dlops& dl)
{
    auto res = active_libraries.find(dl.path());
    if (res != active_libraries.end())
    {
        delete res->second;
        active_libraries.erase(res);
    }
}

void dlops::close_all()
{
    for (auto ptr : active_libraries)
    {
        delete ptr.second;
    }
    active_libraries.clear();
}
